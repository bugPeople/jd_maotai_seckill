...
非常感谢早期作者对该项目的共享，该项目只用于研究所用，对该程序在容器化运行进行了以下优化：

* 优化逻辑，将预约和秒杀在一个参数内运行，目前只需要运行`python main.py -a`即可

* 在参数中增加了秒杀持续时间的设定，到达结束时间后则自动停止循环，默认为3秒

* 增加了run_once参数，后续可以利用公有云函数计算的定时Trigger触发，节省资源

* 替换了Server酱通知到钉钉，便于接受消息

* 增加了Cookie过期检查，如果检查已经过期则自动将重新登陆链接发送至钉钉消息，用户只需要在手机操作即可重新获取Cookie

* 增加了二维码插件，可以直接在Terminal中显示二维码，用户可以直接扫描登陆

## Docker运行方式

原有的config.ini被替换为config.ini.sample模板文件，需要cp为config.ini后，再修改配置参数

直接使用docker-compose运行环境即可
